%% The following code executes the LSADE algorithm with static rules and multiquadratic RBF surrogate
...
for al = [0] % static rules to add poitns based on lipschitz surrogate
for af = [0] % static rules to add poitns based on local optimization
for D = [30,50]    % choice of dimensions
    for funcnr = [1:7]    % choice of function
        ...
        %% Initialization
        c = 1; ker = 'MQ'; pol_part = 0; % choice of RBF
        MaxIt = 1000;       % Maximum number of iterations (not function evaluations)
        MaxFe = 1000;       % Maximum number of function evaluations
        add_LIPO = al; add_RBF = 1; add_FMINCON =af; % choice of adding individual surrogate based points
        ...
        for trial = 1:20    % 20 runs
            ...
            for iter = 1:MaxIt                  % simple iteration counter
                ...
                if add_LIPO > 0 & mod(iter,add_LIPO) == 0 % condition on adding L surrogate children
                ...
                if (add_FMINCON > 0) & mod(iter,add_FMINCON) == 0% condition on adding local optimization surrogate children
                    ...
                    RBF_submodel = rbf_build(X_sub+0.000001*randn(size(X_sub)), s_v(1:quad_vals), ker_sub, c_sub, pol_part_sub, 0); % build local RBF model
